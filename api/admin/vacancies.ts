...
async function saveVacancies(vacancies: any[]): Promise<boolean> {
  const spreadsheetId = process.env.GOOGLE_SHEET_ID;
  const creds = parseCredentials();
...
function requireAuth(req: VercelRequest, res: VercelResponse): string | null {
  ...
  if (req.headers.cookie) {
    const cookies = Object.fromEntries(req.headers.cookie.split(';').map((c) => { const p = c.trim().split('='); return [p[0], p.slice(1).join('=')]; }));
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    try {
      return res.json(await getVacancies());
    } catch (e: any) {
